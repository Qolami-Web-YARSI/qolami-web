        ...
        <>
            <div className='tw-flex tw-mx-auto tw-justify-between tw-w-[100%] tw-pb-16 tw-px-10 md:tw-px-40' dir="ltr">
                {tempIdDetail.indexOf(Number(id)) === 0 ? (
                    <button className=' tw-bg-[#C2EF90] hover:tw-bg-[#90C963] tw-my-5 tw-p-2 tw-rounded-full tw-text-[40px] md:tw-text-[60px]' onClick={load}>
                        <Link to={`/lessons-two/contents/${tempIdDetail[tempIdDetail.length - 1]}`}><FiArrowLeft /></Link></button>
                ) : (
                    <button className=' tw-bg-[#C2EF90] hover:tw-bg-[#90C963] tw-my-5 tw-p-2 tw-rounded-full tw-text-[40px] md:tw-text-[60px]' onClick={load}>
                        <Link to={`/lessons-two/contents/${tempIdDetail[(tempIdDetail.indexOf(Number(id))) - 1]}`}><FiArrowLeft /></Link></button>
                )}
                ...
                {tempIdDetail.indexOf(Number(id)) === tempIdDetail.length - 1 ? (
                    <button className=' tw-bg-[#C2EF90] hover:tw-bg-[#90C963] tw-my-5 tw-p-2 tw-rounded-full tw-text-[40px] md:tw-text-[60px]' onClick={load}>
                        <Link to={`/lessons-two/contents/${tempIdDetail[0]}`}><FiArrowRight /></Link></button>
                ) : (
                    <button className=' tw-bg-[#C2EF90] hover:tw-bg-[#90C963] tw-my-5 tw-p-2 tw-rounded-full tw-text-[40px] md:tw-text-[60px]' onClick={load}>
                        <Link to={`/lessons-two/contents/${tempIdDetail[(tempIdDetail.indexOf(Number(id))) + 1]}`}><FiArrowRight /></Link></button>
                )}
            </div>
        </>
    )
